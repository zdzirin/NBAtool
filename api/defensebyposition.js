...
async function getFullData(year) {
    console.log("defense by position year: ", year);
    // Fetch DBP data from host

    let res = await fetch(`${url}?year=${year - 1}`);
...
async function getDBPFull(year) {
    let data = await getFullData(year);
    return data;
}

export { getDBPByTeam, getDBPFull };
